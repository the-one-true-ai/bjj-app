...
from src.users.schemas import (
    Input_forPublic_UserCreateSchema,
    Input_forSelf_CoachCreateModel,
    Input_forSelf_StudentCreateModel,
    Response_forSelf_CoachProfile,
    Response_forSelf_UserProfile,
    Response_forSelf_StudentProfile,
)
...
class UserService:
    ...
    async def _get_userID_from_studentID(self, student_id: UUID, session: AsyncSession):
        try:
            # Query the dim_students table to get the user_id by student_id
            statement = select(Students.user_id).where(
    ...
    async def get_full_user_profile(self, user_id: UUID, session: AsyncSession):
        try:
            ...
            coach_profile = (
                Response_forSelf_CoachProfile.model_validate(user.coach)
                if user.coach
                else None
            )
            ...
            student_profile = (
                Response_forSelf_StudentProfile.model_validate(user.student)
                if user.student
                else None
            )
...
class CoachService:
    async def get_all_coaches(self, session: AsyncSession):
        try:
            statement = (
                select(Coaches, User).join(  # Select both Coaches and User models
                    User, User.user_id == Coaches.user_id
                )  # Join on user_id
            )
            result = await session.exec(statement)
    ...
    async def get_coach_by_username(self, coach_username: str, session: AsyncSession):
        try:
            ...
            statement = (
                ...
                .join(
                    User, User.user_id == Coaches.user_id
                )  # Ensure you join the related User model
    ...
    async def add_coach_record(
        self,
        user_id: UUID,
        coach_data: Input_forSelf_CoachCreateModel,
...
class StudentService:
    async def get_student_by_username(
        self, student_username: str, session: AsyncSession
    ):
        try:
            statement = (
                select(Students)
                .join(Students.user)
                .where(
                    Students.user.has(username=student_username)
                )  # TODO: Add fuzzy lookup. Limit to Students only
            )
            result = await session.exec(statement)
            return result.all()
        except SQLAlchemyError as e:
    ...
    async def create_student(
        self,
        user_id: UUID,
        student_data: Input_forSelf_StudentCreateModel,
        session: AsyncSession,
    ):
        try:
